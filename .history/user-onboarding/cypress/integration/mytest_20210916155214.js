...
describe("testing app", () => {
  ...
  it("modify input name", () => {
    ...
    cy.wait("@postRequest").then((response) => {
      expect(response.status).property("status").to.equal(201);
      expect(response.data).property("email").to.equal("tom@hank.com");
      expect(response.data).property("password").to.equal("password1");
      expect(response.data).property("name").to.equal("tom");
      expect(response.data).property("termsOfService").to.equal("true");
      expect(response.data).property("role").to.equal("sales");
      expect(response.data).property("id").to.not.equal("");
      expect(response.data).property("createdAt").to.not.equal("");
    });
  });
});
