...
describe("testing app", () => {
  ...
  it("modify input name", () => {
    ...
    cy.wait("@postRequest").then((response) => {
      ...
      expect(response.).property("password").to.equal("password1");
      expect(response.).property("name").to.equal("tom");
      expect(response.).property("termsOfService").to.equal("true");
      expect(response.).property("role").to.equal("sales");
      expect(response.).property("id").to.not.equal("");
      expect(response.).property("createdAt").to.not.equal("");
    });
  });
